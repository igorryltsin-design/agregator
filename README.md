...
- **Backend** — Flask-приложение с SQLAlchemy и PostgreSQL (SQLite поддерживается как legacy/fallback режим). Входная точка `app.py`, REST API вынесен в блюпринт `routes.py`.
...
- **PostgreSQL** — основная база с материалами, тегами, ACL, журналами действий, кешами сниппетов и логами задач.
...
2. Создайте окружение и установите пакеты:
   ...
   ```
3. Установите браузеры Playwright (для headless-поиска достаточно Chromium):
   ```bash
   playwright install chromium
   ```
4. Скопируйте пример `.env` и настройте переменные:
   ```bash
   cp .env.example .env
   ```
...
### OSINT-поиск внутри Docker и доступ к локальным папкам
OSINT-источники с типом `filesystem` читают файлы напрямую через `Path`. В контейнере видны только примонтированные директории (`/app`, `/data` и т.д.), поэтому путь вида `/Users/...` или `\\server\share` с хоста будет недоступен без проброса тома.

1. **Поделитесь папкой с Docker Desktop** (Mac/Windows) или убедитесь, что она существует на хосте (Linux). Для сетевых SMB/NFS-дисков сначала смонтируйте ресурс в систему (например, `/Volumes/research`), затем добавьте его в Settings → Resources → File Sharing.
2. **Примонтируйте путь в контейнер** через `docker-compose.override.yml`:
   ```yaml
   services:
     agregator:
       volumes:
         - ./library:/data               # библиотека по умолчанию
         - /Users/macbook/Docs/osint:/osint-local
         - /Volumes/research-share:/mnt/research
   ```
   После изменения выполните `docker compose up -d` для перезапуска.
3. **Указывайте путь из контейнера** в UI OSINT-поиска. Например, чтобы искать в домашнем каталоге документов, задайте `/osint-local` или `/osint-local/cases/2024`. Относительные пути продолжают работать от `SCAN_ROOT` (= `/data` c дефолтным compose).

Пока том не примонтирован, контейнер физически не видит соответствующие файлы, поэтому локальные и сетевые каталоги останутся пустыми для OSINT-поиска.
...
### Документный чат: план доработок
Работаем последовательно сверху вниз; завершённые пункты отмечаем `✅`, ожидающие реализации — `☐`.

| № | Статус | Задача | Краткое описание |
|---|:------:|--------|------------------|
| 1 | ✅ | Подсветка источников | Автоматически показываем под каждым ответом список задействованных чанков/изображений с цитатами и меткой страницы, чтобы можно было мгновенно сверить ответ с первоисточником. |
| 2 | ✅ | Режимы вопросов | Добавлены переключаемые режимы «Цитата», «Структурный обзор», «Формулы/графики» с разными правилами отбора контекста и подсказками модели. |
| 3 | ✅ | Контекстный диалог | История QA сохраняется и используется при поиске контекста: уточняющие вопросы автоматически дополняются предыдущими репликами и повторно ранжируют ранее задействованные чанки. |
| 4 | ✅ | Персонификация | Пользователь задаёт тон, детализацию и язык ответа; предпочтения сохраняются и автоматически учитываются при генерации и ранжировании контекста. |
| 5 | ✅ | Расширенный vision/OCR | Каждое изображение проходит captioning и OCR (pytesseract), распознанный текст добавляется в контекст и выдаётся в UI вместе с координатами слов. |
| 6 | ✅ | Пост-валидация ответов | Ответы автоматически проверяются на наличие ссылок/ошибок, вычисляется риск (low/medium/high), а предупреждения показываются прямо в чате. |

### План работ: OSINT-поиск
- [x] Усилить сводное ранжирование и дедупликацию (`agregator/osint/service.py`) — единый скоринг по источникам, учёт свежести и нормализация URL.
- [x] Улучшить LLM-аналитику: структурированный шаблон отчёта, повторное использование `llm_payload`, вывод предупреждений и фиксация ошибок (`agregator/osint/service.py`, `tests/test_osint_analysis.py`).
- [x] Автоматизировать повторные поисковые задачи и уведомления: хранилище расписаний, фоновой воркер, REST-обновление и UI для управления (`agregator/osint/storage.py`, `agregator/osint/service.py`, `app.py`, `frontend/src/pages/OsintPage.tsx`).
- [x] Расширить обработку локальных источников: подсветка совпадений, OCR и фильтры исключений (`agregator/osint/service.py`, `frontend/src/pages/OsintPage.tsx`, `tests/test_osint_analysis.py`).
- [x] Подсказать о ручной капче прямо в UI: toast с кнопкой перехода и предупреждения в карточках источников (SERP fallback) (`frontend/src/ui/Toasts.tsx`, `frontend/src/pages/OsintPage.tsx`).
- [x] Очищать запрос до ключевых слов и показывать статистику по страницам/ссылкам для каждого источника (`agregator/osint/service.py`, `agregator/osint/storage.py`, `frontend/src/pages/OsintPage.tsx`).
- [x] Обогащать артефакты источников: текстовые извлечения, скриншоты, структурированные метаданные (`agregator/osint/storage.py`, `agregator/osint/serp.py`, `frontend/src/pages/OsintPage.tsx`).
- [x] Экспортировать отчёт по задаче (Markdown-файл, кнопка в UI) (`agregator/osint/service.py`, `app.py`, `frontend/src/pages/OsintPage.tsx`).
- [x] Добавить выгрузку OSINT-отчёта в JSON (API + кнопка) (`app.py`, `frontend/src/pages/OsintPage.tsx`).
- [x] Повторный запуск источника после капчи (ссылка, диалог, API) (`agregator/osint/service.py`, `app.py`, `frontend/src/pages/OsintPage.tsx`).
