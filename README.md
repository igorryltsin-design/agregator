...
| **Метаданные и фильтры** | ☐ | Расширить структурный контекст: отдельные блоки «по авторам», «по годам», генерация подсказок в UI. |
| **Граф авторитетности** | ☐ | Построить PageRank/TopicRank по ссылкам, тегам и коллекциям. Использовать веса графа в ранжировании и аналитике (раздел статистики). |
| **Обучение на фидбэке** | ☐ | Собрать лог кликов/оценок, запускать ночной тренинг, синхронизировать веса rerank'а. |
| **CI/CD и мониторинг** | ☐ | Авто-прогон smoke-тестов RAG (`scripts/rag_smoke.py`), сбор метрик nDCG/MRR и алерты. |
...
- Автоматизация деплоя и CI/CD (контейнеризация, прогон тестов, контроль качества данных).
