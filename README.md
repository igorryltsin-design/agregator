...
docker compose up -d
```
Контейнер слушает порт `5050`, монтирует `./library` в `/data` и `./backups` в `/app/backups`. Переменные окружения вынесены в `docker/app.env`, дополнительные переопределения можно хранить в `docker-compose.override.yml`. Для ручной сборки образа см. `Dockerfile`.
По умолчанию Compose собирает и запускает образ как `linux/amd64`, поэтому на Apple Silicon (M1/M2) получится сразу получить x64-сборку; при необходимости измените поле `platform` в `docker-compose.yml`.
...
- [x] Разбить `Agregator/app.py` на модули (app_factory, routes, services, tasks, models/utils) и собрать через Flask factory. _(добавлены `setup_app` и фабрика `agregator.create_app`)_
- [x] Перенести конфигурацию в отдельный модуль (Pydantic или dataclass) с явным чтением переменных окружения и дефолтами. _(реализован `AppConfig` в `agregator/config.py`)_
- [x] Зарезервировать пакет `agregator/services/…` для обработки файлов, интеграций с LLM, очередей. _(создан пакет сервисов и вынесены хелперы логирования)_
Инфраструктура и устойчивость

- [x] Ввести очередь задач (Celery/RQ/APScheduler) для OCR, Whisper, чтобы веб-процесс не блокировался. _(реализована внутренняя очередь `TaskQueue`, управляющая сканированием)_
- [x] Стандартизировать HTTP-клиент: обернуть requests в функцию с retry, таймаутами, логированием и graceful fallback. _(реализован сервис `http_request` и конфигурация `HttpSettings`)_
- [ ] Настроить единый логгер (структурные записи + RotatingFileHandler/Sentry) с фильтрацией чувствительных данных.
- [ ] Добавить health-check эндпоинт и metrics (Prometheus) для фоновых задач и очередей.

Функциональные улучшения
8. Вынести управление пользователями/ролями в отдельный Blueprint, расширить RBAC (админ, редактор, просмотр).

10. Расширить интеграции LLM: поддержка нескольких провайдеров, режим «чтение из кэша», лимиты токенов.
11. Улучшить поиск/фасеты: отдельный сервис индексации, подсветка совпадений, кеширование результатов.
12. Для импорта файлов — очередь, превью, прогресс-бары, автоматическая генерация метаданных (язык, тематика).

Тестирование и качество

15. Подготовить docker-compose для dev/test, снабдить README подробными шагами запуска и env-примером.

UI/UX
16. Обновить документацию (README. Agregator.md) с архитектурой, диаграммой потоков, инструкцией по локальной настройке и деплою.


Очередность: сначала структуру и конфигурацию, затем устойчивость (очереди, retry), далее функциональные улучшения и тесты.
