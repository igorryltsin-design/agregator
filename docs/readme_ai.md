...
- [ ] **Проверка ссылок**: убедиться, что каждая строка блока «Факты» содержит ссылку `[doc_id:chunk]`; при отсутствии — помечать ответ как `suspected_hallucination` и триггерить предупреждение в логах.
- [ ] **Согласованность**: валидация, что цитируемые чанки действительно присутствуют в контексте; если LLM сослался на несуществующий chunk, удалять этот факт и добавлять в предупреждения.
- [ ] **Fallback**: если dense-индекс недоступен, переключаться на SQL/BM25 и явно отмечать в прогрессе «Работаем в fallback-режиме, только ключевые слова».
- [ ] **Пустой ответ**: при низком confidence (например, максимум контекстных скор <0.25 или нет ссылок) возвращать шаблон «Источники не найдены, попробуйте переформулировать запрос».


### UX и аналитика
- [ ] **Интерфейс**: карточка «Ответ RAG» с разделом «Факты» и кликабельными ссылками `[doc_id:chunk]`, подсветкой фрагментов в сниппете документа, кнопкой «Открыть источник».
- [ ] **Статус выполнения**: отображать прогресс-бар с этапами «Выбор кандидатов», «Rerank», «Формирование ответа», «Проверка ссылок»; при fallback добавлять предупреждение.
- [ ] **История запросов**: сохранять последние N RAG-сессий пользователя, позволять повторно открыть контекст и поделиться ссылкой на ответ.
- [ ] **Метрики**: считать время на этапах (ingest, retrieve, rerank, generate), долю ответов с подозрениями (hallucinations), среднее число источников, coverage документов.

### Тестирование и эксплуатация
- [ ] **Unit-тесты**: сегментация, нормализация, чанкование (проверка overlap), сериализация промпта, валидация ссылок, поведение на пустых/шумных документах.
- [ ] **Интеграционные тесты**: end-to-end пайплайн «импорт → индекс → поиск → генерация», сценарии с мультиязычными документами и fallbacks без эмбеддингов.
- [ ] **Smoke-скрипт**: CLI, который прогоняет ~10 типовых запросов, проверяет, что есть ответ с источниками, и логирует длительности; использовать в CI.
- [ ] **Утилиты эксплуатации**: CLI `rag-index rebuild`, `rag-index diff --from v1 --to v2`, `rag-index inspect --chunk <id>`; cron для отчета о неуспешных импортов.
- [ ] **Документация**: поддерживать схемы таблиц (ER-диаграмма), чек-листы по обновлению моделей, инструкции по мониторингу и реагированию на ошибки индексации.
